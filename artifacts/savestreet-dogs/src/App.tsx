...
function Products({ cart, setCart }: { cart: CartLine[]; setCart: Dispatch<SetStateAction<CartLine[]>> }) {
  ...
  const add = (product: Product) => setCart(items => { const found = items.find(item => item.id === product.id); return found ? items.map(item => item.id === product.id ? { ...item, quantity: item.quantity + 1 } : item) : [...items, { ...product, quantity: 1 }]; });
...
function Stories() {
  ...
  return <><Meta title="Field stories" description="Illustrative field notes about the patient, practical work of helping street dogs." /><main><PageHeader eyebrow="Field notes" title={<>The stories<br /><em>stay with us.</em></>} text="Real rescue work is made of ordinary moments. These clearly labeled illustrative notes explore what care can look like without turning animals into before-and-after content." /><section className="content-section container"><div className="story-grid" style={{ color: '#173833' }}>{stories.map((story, index) => <article className="story-card" style={{ borderColor: '#dcd4c6' }} key={story.title}><img src={story.image} alt={`${story.title}, illustrative story`} /><div className="eyebrow">{story.label} · demo content</div><h2 style={{ color: '#173833' }}>{story.title}</h2><p style={{ color: '#526a64' }}>A short, illustrative note about the details that help trust grow: time, routine, and one person willing to pay attention.</p><button className="btn btn-ghost" style={{ marginTop: 17 }} onClick={() => alert('This illustrative story will be expanded in a future release.')} data-testid={`button-story-${index}`}>Read the note <ArrowRight size={14} /></button></article>)}</div></section></main></>;
}
