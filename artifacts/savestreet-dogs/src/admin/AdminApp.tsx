...
import {
  ...
  useUpdateAdoptionRequest, useUpdateAdminOrder, useUpdateProduct, useUpdatePuppy, useVerifyOrderPayment,
...
const money = (rupees = 0) => `₹${rupees.toFixed(0)}`;
...
const emptyProduct: ProductInput = { name: "", description: "", category: "", priceRupees: 60, stock: 0, available: true, imageUrls: [] };

function UploadField({ images, onUpload }: { images: string[]; onUpload: (file: File) => Promise<void> }) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [pendingPreview, setPendingPreview] = useState<{ url: string; name: string } | null>(null);
  async function choose(file?: File) {
    if (!file) return;
    const previewUrl = URL.createObjectURL(file);
    setPendingPreview({ url: previewUrl, name: file.name });
  ...
  return <div className="admin-upload"><div style={{ display: "flex", alignItems: "center", gap: 8 }}><ImagePlus size={16} /><strong style={{ fontSize: 12 }}>Add an image</strong><span style={{ color: "var(--admin-muted)", fontSize: 11 }}>saved to persistent storage</span></div><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => void choose(event.target.files?.[0])} disabled={busy} data-testid="input-image-upload" />{busy && <small>Uploading image…</small>}{error && <small style={{ color: "var(--admin-red)" }}>{error}</small>}{images.length > 0 && <small>{images.length} image{images.length === 1 ? "" : "s"} ready to save.</small>}<div className="admin-image-strip">{pendingPreview && <div className="admin-image-preview-pending"><img src={pendingPreview.url} alt={`Preview of ${pendingPreview.name}`} /><small>Preview</small></div>}{images.map((image) => <img src={objectUrl(image)} alt="" key={image} data-testid={`img-upload-${image.replace(/[^a-zA-Z0-9]/g, "-")}`} />)}</div></div>;
}
...
function PuppyEditor({ selected, onDone }: { selected: Puppy | null; onDone: (message: string, error?: string) => void }) {
  ...
  return <form className="admin-panel admin-form" onSubmit={submit} data-testid="form-puppy"><h2>{selected ? "Edit puppy" : "Add a puppy"}</h2><p className="admin-form-intro">Keep this profile specific. The public listing is powered by these fields.</p><div className="admin-form-grid">
    ...
    <Field label="Description" full><textarea value={form.description} onChange={(e) => set("description", e.target.value)} data-testid="textarea-puppy-description" /></Field><Field label="Adoption information"><textarea value={form.adoptionInfo} onChange={(e) => set("adoptionInfo", e.target.value)} data-testid="textarea-puppy-adoption-info" /></Field><Field label="Health information"><textarea value={form.healthInfo} onChange={(e) => set("healthInfo", e.target.value)} data-testid="textarea-puppy-health-info" /></Field><Field label="Vaccination information"><textarea value={form.vaccinationInfo} onChange={(e) => set("vaccinationInfo", e.target.value)} data-testid="textarea-puppy-vaccination-info" /></Field><Field label="Rescue story"><textarea value={form.rescueStory} onChange={(e) => set("rescueStory", e.target.value)} data-testid="textarea-puppy-rescue-story" /><small>Saved when the backend supports this field.</small></Field><Field label="Internal notes" full><textarea value={form.notes} onChange={(e) => set("notes", e.target.value)} data-testid="textarea-puppy-notes" /></Field>
     <Field label="Photos" full><UploadField images={form.imageUrls || []} onUpload={async (file) => { const imageUrl = await uploadAdminImage(file); setForm((current) => ({ ...current, imageUrls: [...(current.imageUrls || []), imageUrl] })); }} /></Field>
...
function ProductEditor({ selected, onDone }: { selected: Product | null; onDone: (message: string, error?: string) => void }) {
  ...
  useEffect(() => { setForm(selected ? { ...selected } : { ...emptyProduct }); }, [selected]);
  const set = (key: keyof ProductInput, value: string | number | boolean | string[]) => setForm((current) => ({ ...current, [key]: value }));
  async function submit(event: FormEvent) { event.preventDefault(); try { const body: ProductInput = { ...form, priceRupees: Number(form.priceRupees || 0), stock: Number(form.stock || 0), available: Number(form.stock || 0) > 0 && Boolean(form.available), imageUrls: form.imageUrls || [] }; if (selected) await update.mutateAsync({ id: selected.id, data: body }); else await create.mutateAsync({ data: body }); await queryClient.invalidateQueries({ queryKey: getListAdminProductsQueryKey() }); await queryClient.invalidateQueries({ queryKey: getGetAdminDashboardQueryKey() }); onDone(selected ? "Product updated." : "Product added."); } catch (error) { onDone("", apiErrorMessage(error)); } }
  const stockStatus = Number(form.stock || 0) === 0 ? "No stock" : Number(form.stock || 0) < 5 ? "Low stock" : "In stock";
  return <form className="admin-panel admin-form" onSubmit={submit} data-testid="form-product"><h2>{selected ? "Edit product" : "Add a product"}</h2><p className="admin-form-intro">Prices are entered and stored as Indian rupees. Availability follows stock and the explicit availability toggle.</p><div className="admin-form-grid"><Field label="Name *" full><input value={form.name} onChange={(e) => set("name", e.target.value)} required data-testid="input-product-name" /></Field><Field label="Category"><input value={form.category} onChange={(e) => set("category", e.target.value)} data-testid="input-product-category" /></Field><Field label="Stock"><input type="number" min="0" value={form.stock} onChange={(e) => set("stock", Number(e.target.value))} data-testid="input-product-stock" /></Field><Field label="Price (₹)"><input type="number" min="0" step="1" value={form.priceRupees} onChange={(e) => set("priceRupees", Number(e.target.value))} data-testid="input-product-price" /><small>Default price: ₹60 per piece.</small></Field><Field label="Stock status"><div style={{ paddingTop: 9 }}><Badge value={stockStatus} /></div></Field><Field label="Description" full><textarea value={form.description} onChange={(e) => set("description", e.target.value)} data-testid="textarea-product-description" /></Field><Field label="Availability" full><label style={{ display: "flex", alignItems: "center", gap: 8 }}><input type="checkbox" checked={Boolean(form.available)} onChange={(e) => set("available", e.target.checked)} data-testid="checkbox-product-available" /> Visible in shop when stock is available</label></Field><Field label="Product image" full><UploadField images={form.imageUrls || []} onUpload={async (file) => { const imageUrl = await uploadAdminImage(file); setForm((current) => ({ ...current, imageUrls: [...(current.imageUrls || []), imageUrl] })); }} /></Field></div><div className="admin-form-actions"><button className="admin-button" disabled={!form.name.trim() || create.isPending || update.isPending} data-testid="button-save-product">{create.isPending || update.isPending ? "Saving…" : selected ? "Save changes" : "Add product"} <Check size={14} /></button></div></form>;
}
...
function OrdersPage() {
  const query = useListAdminOrders({ query: { queryKey: getListAdminOrdersQueryKey() } }); const update = useUpdateAdminOrder(); const verify = useVerifyOrderPayment(); const queryClient = useQueryClient(); const [selected, setSelected] = useState<Order | null>(null); const [message, setMessage] = useState("");
  async function save(id: string, payload: { status: string; deliveryNotes?: string; expectedDelivery?: string | null; delayReason?: string; deliveryPerson?: string; deliveryPhone?: string; trackingId?: string }) { try { await update.mutateAsync({ id, data: payload }); await queryClient.invalidateQueries({ queryKey: getListAdminOrdersQueryKey() }); setMessage("Order updated."); setSelected(null); } catch (error) { setMessage(apiErrorMessage(error)); } }
  async function verifyPayment(order: Order) { try { await verify.mutateAsync({ id: order.id, data: { status: "Payment Verified" } }); await queryClient.invalidateQueries({ queryKey: getListAdminOrdersQueryKey() }); setMessage(`Payment verified for ${order.orderCode}.`); setSelected(null); } catch (error) { setMessage(apiErrorMessage(error)); } }
  return <><PageTitle eyebrow="Mission shop" title="Orders" text="Verify payment, coordinate delivery, and keep each customer handoff current." />{message && <Notice kind={message.includes("updated") || message.includes("verified") ? "success" : "error"}>{message}</Notice>}<section className="admin-panel">{query.isLoading ? <LoadingState /> : query.error ? <ErrorState error={query.error} retry={() => void query.refetch()} /> : !query.data?.length ? <EmptyState title="No orders yet." text="Orders appear after a verified checkout is created." /> : <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Order</th><th>Customer</th><th>Total</th><th>Payment</th><th>Delivery</th><th /></tr></thead><tbody>{query.data.map((order) => <tr key={order.id} data-testid={`row-order-${order.id}`}><td><div className="admin-record-name">{order.orderCode}</div><div className="admin-record-sub">{niceDate(order.createdAt)}</div></td><td><div className="admin-record-name">{order.customerName}</div><div className="admin-record-sub">{order.city}</div></td><td>{money(order.totalRupees)}</td><td><Badge value={order.paymentStatus} /></td><td><Badge value={order.status} /></td><td><div className="admin-actions"><button className="admin-button secondary small" onClick={() => setSelected(order)} data-testid={`button-review-order-${order.id}`}>Open <ChevronRight size={12} /></button>{order.paymentStatus === "Payment Pending" && <button className="admin-button small" onClick={() => void verifyPayment(order)} disabled={verify.isPending} data-testid={`button-verify-payment-${order.id}`}>Verify payment</button>}</div></td></tr>)}</tbody></table></div>}</section>{selected && <OrderEditor order={selected} onClose={() => setSelected(null)} onSave={save} onVerify={() => void verifyPayment(selected)} verifying={verify.isPending} />}</>;
}

function OrderEditor({ order, onClose, onSave, onVerify, verifying }: { order: Order; onClose: () => void; onSave: (id: string, payload: { status: string; deliveryNotes?: string; expectedDelivery?: string | null; delayReason?: string; deliveryPerson?: string; deliveryPhone?: string; trackingId?: string }) => Promise<void>; onVerify: () => void; verifying: boolean }) {
  const [status, setStatus] = useState(order.status); const [notes, setNotes] = useState(order.deliveryNotes || ""); const [delayReason, setDelayReason] = useState(order.delayReason || ""); const [person, setPerson] = useState(order.deliveryPerson || ""); const [phone, setPhone] = useState(order.deliveryPhone || ""); const [tracking, setTracking] = useState(order.trackingId || ""); const [expected, setExpected] = useState(order.expectedDelivery ? order.expectedDelivery.slice(0, 10) : "");
  return <div className="admin-panel admin-detail" style={{ marginTop: 15 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><div><div className="admin-eyebrow">Order detail</div><h3>{order.orderCode}</h3></div><button className="admin-button secondary small" onClick={onClose} data-testid="button-close-order"><X size={13} /> Close</button></div><div className="admin-detail-row"><label>Customer</label><span>{order.customerName}<br />{order.email} · {order.phone}<br />{order.address}, {order.city}, {order.pinCode}</span></div><div className="admin-detail-row"><label>Total</label><span>{money(order.totalRupees)}</span></div><div className="admin-detail-row"><label>Payment</label><span><Badge value={order.paymentStatus} /> {order.paymentStatus === "Payment Pending" && <button className="admin-button small" onClick={onVerify} disabled={verifying} data-testid="button-verify-payment-detail">{verifying ? "Verifying…" : "Verify payment"}</button>}</span></div><div className="admin-form-grid"><Field label="Delivery status"><select value={status} onChange={(e) => setStatus(e.target.value as typeof status)} data-testid="select-order-status"><option>Payment Pending</option><option>Payment Verified</option><option>Preparing</option><option>Ready to Dispatch</option><option>Dispatched</option><option>Out for Delivery</option><option>Delivered</option><option>Delayed</option><option>Cancelled</option></select></Field><Field label="Expected delivery"><input type="date" value={expected} onChange={(e) => setExpected(e.target.value)} data-testid="input-order-expected" /></Field><Field label="Tracking ID"><input value={tracking} onChange={(e) => setTracking(e.target.value)} data-testid="input-order-tracking" /></Field><Field label="Delivery person"><input value={person} onChange={(e) => setPerson(e.target.value)} data-testid="input-order-delivery-person" /></Field><Field label="Delivery phone"><input value={phone} onChange={(e) => setPhone(e.target.value)} data-testid="input-order-delivery-phone" /></Field><Field label="Delay reason"><input value={delayReason} onChange={(e) => setDelayReason(e.target.value)} data-testid="input-order-delay-reason" /></Field><Field label="Delivery notes" full><textarea value={notes} onChange={(e) => setNotes(e.target.value)} data-testid="textarea-order-notes" /></Field></div><div className="admin-form-actions"><button className="admin-button" onClick={() => void onSave(order.id, { status, deliveryNotes: notes, expectedDelivery: expected || null, delayReason, deliveryPerson: person, deliveryPhone: phone, trackingId: tracking })} data-testid="button-save-order">Save order</button></div></div>;
}
...
function GenericPage({ title, eyebrow, text, path, actionLabel }: { title: string; eyebrow: string; text: string; path: string; actionLabel?: string }) {
  ...
  async function update(record: GenericRecord, patch: Record<string, unknown>) { if (!record.id) return; try { const requestPatch = title === "Stories" ? { published: patch.status === "Published" } : patch; await adminJson(`${path}/${record.id}`, { method: "PATCH", body: JSON.stringify(requestPatch) }); await queryClient.invalidateQueries({ queryKey: [path] }); setMessage("Record updated."); setSelected(null); } catch (error) { setMessage(apiErrorMessage(error)); } }
...
function GenericCreateForm({ title, onClose, onSave }: { title: string; onClose: () => void; onSave: (payload: Record<string, unknown>) => Promise<void> }) {
  ...
  return <form className="admin-panel admin-form" style={{ marginBottom: 15 }} onSubmit={(event) => { event.preventDefault(); void onSave({ slug: name.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, ""), title: name, excerpt: description, body: description, imageUrl: "", published: status === "Published" }); }} data-testid={`form-create-${title.toLowerCase()}`}><div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><div><div className="admin-eyebrow">New {title.toLowerCase().slice(0, -1)}</div><h2>Create record</h2></div><button type="button" className="admin-button secondary small" onClick={onClose} data-testid="button-cancel-create"><X size={13} /> Cancel</button></div><div className="admin-form-grid"><Field label="Title or name" full><input value={name} onChange={(e) => setName(e.target.value)} required data-testid="input-generic-name" /></Field><Field label="Copy or description" full><textarea value={description} onChange={(e) => setDescription(e.target.value)} data-testid="textarea-generic-description" /></Field><Field label="Status"><select value={status} onChange={(e) => setStatus(e.target.value)} data-testid="select-generic-status"><option>Unpublished</option><option>Published</option></select></Field></div><div className="admin-form-actions"><button className="admin-button" data-testid="button-save-generic">Create {title.toLowerCase().slice(0, -1)} <Check size={14} /></button></div></form>;
}
...
function GenericEditor({ record, title, onClose, onSave }: { record: GenericRecord; title: string; onClose: () => void; onSave: (patch: Record<string, unknown>) => Promise<void> }) {
  const [status, setStatus] = useState(String(record.status ?? (record.published ? "Published" : "Unpublished"))); const [notes, setNotes] = useState(String(record.internalNotes ?? record.notes ?? ""));
  return <div className="admin-panel admin-detail" style={{ marginTop: 15 }}><div style={{ display: "flex", justifyContent: "space-between" }}><div><div className="admin-eyebrow">{title} detail</div><h3>{String(record.name ?? record.title ?? record.applicantName ?? "Record")}</h3></div><button className="admin-button secondary small" onClick={onClose} data-testid="button-close-record"><X size={13} /> Close</button></div><div className="admin-detail-row"><label>Message</label><span>{String(record.message ?? record.details ?? record.additionalMessage ?? record.body ?? "No message supplied.")}</span></div><div className="admin-form-grid"><Field label="Status"><select value={status} onChange={(e) => setStatus(e.target.value)} data-testid="select-record-status"><option>New</option><option>Reviewing</option><option>Assigned</option><option>In Progress</option><option>Under Review</option><option>Contacted</option><option>Approved</option><option>Not Approved</option><option>Completed</option><option>Resolved</option><option>Closed</option><option>Published</option><option>Unpublished</option></select></Field><Field label="Internal notes"><textarea value={notes} onChange={(e) => setNotes(e.target.value)} data-testid="textarea-record-notes" /></Field></div><div className="admin-form-actions"><button className="admin-button" onClick={() => void onSave({ status, internalNotes: notes })} data-testid="button-save-record">Save update</button></div></div>;
}
...
function AdminApp() {
  ...
  else if (page === "/admin/volunteers") content = <GenericPage title="Volunteers" eyebrow="People power" text="Review interest, record the next conversation, and keep reliable helpers close." path="/api/admin/volunteer-applications" actionLabel="Add volunteer" />;
  else if (page === "/admin/foster") content = <GenericPage title="Foster" eyebrow="Temporary homes" text="Track foster interest and the follow-up that helps dogs settle safely." path="/api/admin/foster-applications" actionLabel="Add foster" />;
