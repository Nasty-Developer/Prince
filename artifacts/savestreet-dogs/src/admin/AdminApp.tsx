...
function OrderEditor({ order, onClose, onSave }: { order: Order; onClose: () => void; onSave: (id: string, payload: { status: string; paymentStatus?: string; deliveryNotes?: string; expectedDelivery?: string | null; deliveryPerson?: string; deliveryPhone?: string; trackingId?: string }) => Promise<void> }) {
  ...
  return <div className="admin-panel admin-detail" style={{ marginTop: 15 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><div><div className="admin-eyebrow">Order detail</div><h3>{order.orderCode}</h3></div><button className="admin-button secondary small" onClick={onClose} data-testid="button-close-order"><X size={13} /> Close</button></div><div className="admin-detail-row"><label>Customer</label><span>{order.customerName}<br />{order.email} · {order.phone}<br />{order.address}, {order.city}, {order.pinCode}</span></div><div className="admin-detail-row"><label>Total</label><span>{money(order.totalRupees)}</span></div><div className="admin-form-grid"><Field label="Payment"><select value={payment} onChange={(e) => setPayment(e.target.value as typeof payment)} data-testid="select-order-payment"><option>Pending</option><option>Paid</option><option>Failed</option><option>Refunded</option></select></Field><Field label="Delivery status"><select value={status} onChange={(e) => setStatus(e.target.value as typeof status)} data-testid="select-order-status"><option>Pending</option><option>Preparing</option><option>Dispatched</option><option>Delivered</option><option>Delayed</option><option>Cancelled</option></select></Field><Field label="Expected delivery"><input type="date" value={expected} onChange={(e) => setExpected(e.target.value)} data-testid="input-order-expected" /></Field><Field label="Tracking ID"><input value={tracking} onChange={(e) => setTracking(e.target.value)} data-testid="input-order-tracking" /></Field><Field label="Delivery person"><input value={person} onChange={(e) => setPerson(e.target.value)} data-testid="input-order-delivery-person" /></Field><Field label="Delivery phone"><input value={phone} onChange={(e) => setPhone(e.target.value)} data-testid="input-order-delivery-phone" /></Field><Field label="Delivery notes" full><textarea value={notes} onChange={(e) => setNotes(e.target.value)} data-testid="textarea-order-notes" /></Field></div><div className="admin-form-actions"><button className="admin-button" onClick={() => void onSave(order.id, { status, paymentStatus: payment, deliveryNotes: notes, expectedDelivery: expected || null, deliveryPerson: person, deliveryPhone: phone, trackingId: tracking })} data-testid="button-save-order">Save order</button></div></div>;
}
