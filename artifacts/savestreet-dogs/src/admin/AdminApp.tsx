...
function UploadField({ images, onUpload }: { images: string[]; onUpload: (file: File) => Promise<void> }) {
  ...
  return <div className="admin-upload"><div style={{ display: "flex", alignItems: "center", gap: 8 }}><ImagePlus size={16} /><strong style={{ fontSize: 12 }}>Add an image</strong><span style={{ color: "var(--admin-muted)", fontSize: 11 }}>saved to Firebase Storage when you save</span></div><input type="file" accept="image/*" onChange={(event) => void choose(event.target.files?.[0])} disabled={busy} data-testid="input-image-upload" />{busy && <small>Uploading image…</small>}{error && <small style={{ color: "var(--admin-red)" }}>{error}</small>}{images.length > 0 && <small>{images.length} image{images.length === 1 ? "" : "s"} ready to save.</small>}<div className="admin-image-strip">{images.map((image) => <img src={objectUrl(image)} alt="" key={image} data-testid={`img-upload-${image.replace(/[^a-zA-Z0-9]/g, "-")}`} />)}</div></div>;
}
